<!-- Content Wrapper. Contains page content -->
  <div class="content-wrapper">
    ...
    <section class="content-header">
      <h1>
       <?=$header?>
       <a class="btn btn-primary" href="<?=base_url();?>ita/create"><i class="fa fa-plus"></i> เพิ่มข้อมูล ita</a>
      </h1>
      <ol class="breadcrumb">
  ...
  <script type="text/javascript">
    $(function(){
      ...
      var table = $('#table').DataTable( {
        ...
        "columns": [          
            { "data": function (data, type, row, meta) {
              return '<input type="checkbox" class="btn-dlm" name="chk_ita[]" value="'+data.id+'"> ';
            }},
            { "data": "title" },
            { "data": function(data){
            ...
            { "data":function(data){
              if(data.status!=0){
                return "<input type='checkbox' class='show_ita' checked>";
              }else{
                return "<input type='checkbox' class='show_ita' >";
              }      
            } },
            {"data": function(data, type, row, meta){
              return   "<div class='btn-group'>"+
              "<a href='<?=base_url();?>ita/edit/"+data.id+" ' class='btn btn-default btn-sm edit'><i class='fa fa-edit'></i></a>"+
      ...
      function cb(start='', end='') { 
        ...
        if(start==''){
          table.ajax.url('<?=base_url();?>ita/ita_list').load();   
          ...
          table.ajax.url('<?=base_url();?>ita/ita_list/?'+$('#daterange').serialize()).load(); 
        }     
      }
      ...
      $(document).on('click','#delete_multi',function(){
        ...
        if(!$('.btn-dlm').is(':checked')){
          swal('Warning','Please choose ita to delete','warning');
          return false;
        }
      ...
      $(document).on('click', '.show_ita', function(event) {
        var status = 0;
        var data = table.row( $(this).parents('tr') ).data();
        if(!$(this).is(':checked')){
          status = 0;
