<!-- Content Wrapper. Contains page content -->
  <div class="content-wrapper">
    ...
    <section class="content-header">
      <h1>
       <?=$header?>
       <a class="btn btn-primary" href="<?=base_url();?>news/create"><i class="fa fa-plus"></i> เพิ่มข่าวสารประชาสัมพันธ์ อบต.</a>
      </h1>
      <ol class="breadcrumb">
  ...
  <script type="text/javascript">
    $(function(){
      ...
      var table = $('#table').DataTable( {
        ...
        "columns": [          
            { "data": function (data, type, row, meta) {
              return '<input type="checkbox" class="btn-dlm" name="chk_news[]" value="'+data.id+'"> ';
            }},
            { "data": "title" },
            { "data": function(data){
            ...
            { "data":function(data){
              if(data.status!=0){
                return "<input type='checkbox' class='show_news' checked>";
              }else{
                return "<input type='checkbox' class='show_news' >";
              }      
            } },
            {"data": function(data, type, row, meta){
              return   "<div class='btn-group'>"+
              "<a href='<?=base_url();?>news/edit/"+data.id+" ' class='btn btn-default btn-sm edit'><i class='fa fa-edit'></i></a>"+
      ...
      function cb(start='', end='') { 
        ...
        if(start==''){
          table.ajax.url('<?=base_url();?>news/news_list').load();   
          ...
          table.ajax.url('<?=base_url();?>news/news_list/?'+$('#daterange').serialize()).load(); 
        }     
      }
      ...
      $(document).on('click','#delete_multi',function(){
        ...
        if(!$('.btn-dlm').is(':checked')){
          swal('Warning','Please choose news to delete','warning');
          return false;
        }
        ...
        swal({
          ...
          preConfirm: function () {
            return new Promise(function (resolve) {
              $.ajax({
                url: '<?=base_url();?>news/news_delmulti',
                type: 'POST',
                data: form_data,
                success:function(data){
                  console.log(data);
      ...
      $(document).on('change', '.order_by', function(event) {
        ...
        var order_by = $(this).val();
              ...
              $.ajax({
                url: '<?=base_url();?>news/news_order_by',
                type: 'POST',
      ...
      $(document).on('click', '.show_news', function(event) {
        var status = 0;
        var data = table.row( $(this).parents('tr') ).data();
        if(!$(this).is(':checked')){
          status = 0;
