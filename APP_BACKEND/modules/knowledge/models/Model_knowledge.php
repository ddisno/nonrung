<?php
	...
	class Model_knowledge extends CI_Model
	{
		...
		public function knowledge_list($val){
			$this->db->select('*');
			...
			if($val!=''){
				...
				$this->db->where('knowledge.create_datetime >=',$date_start);
				$this->db->where('knowledge.create_datetime <=',$date_end);
			}
		...
		public function save_knowledge($data){
			$get = $this->db->insert('knowledge',$data);
			if($get){
				return true;
			}else{
				return false;
		...
		public function edit_knowledge($data,$id){
			$this->db->where('id',$id);
		...
		// ลบบล็อก--------------
		public function knowledge_del($id){
			$this->db->where('id',$id);
		...
		// -----end knowledge del single

		public function get_order_knowledge(){
			$this->db->select('*');
		...
		public function knowledge_status($data,$id){
			$this->db->where('id',$id);
			$query = $this->db->update('knowledge',$data);
			if($query){
				return true;
		...
		public function knowledge_delmulti($data){
			if (!empty($data)) {
		...
		public function knowledge_status_home($data,$id){
			$this->db->where('id_cat',$id);
			$query = $this->db->update('knowledge_cat',$data);
			if($query){
				return true;
			}else{
				return false;
		...
		// --------------IF EXSIT TABLE------------------
		public function knowledge_table(){
			// create knowledge table
			$fields_knowledge = array(
		        'id' => array(
		                'type' => 'INT',
		                'auto_increment' => TRUE
		        ),
			...
			$this->dbforge->add_field($fields_knowledge);
			...
			$this->dbforge->create_table('knowledge',TRUE,$attributes);

			// $this->db->query('ALTER TABLE knowledge ADD FOREIGN KEY(id_cat) REFERENCES knowledge_cat(id_cat) ON DELETE RESTRICT ON UPDATE RESTRICT;');
		}
	// end exsit table
	}
?>