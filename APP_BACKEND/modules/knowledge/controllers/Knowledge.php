...
class Knowledge extends MY_Controller
{
	...
	public function index(){
		...
		$this->data['title'] = 'ความรู้ทั่วไป';
		$this->data['header'] = 'ความรู้ทั่วไป';
		$this->middle = 'manage';
		$this->layout();
	}
	...
	public function create($id=''){
		if(null!==$this->input->server('REQUEST_METHOD') && $this->input->server('REQUEST_METHOD') == 'POST'){
			...
			$get_order = $this->model_knowledge->get_order_knowledge();
			if($get_order){
	        ...
	        $get = $this->model_knowledge->save_knowledge($data);
	        if($get){
		...
		$this->data['title'] = 'knowledge Create';
		$this->data['header'] = 'เพิ่มความรู้ทั่วไป';
		$this->middle = 'create';
		$this->layout();
	}

	public function edit($id=''){
		if($get = $this->db->where('id',$this->uri->segment(3))->get('knowledge')->num_rows()==''){
	      redirect('knowledge');
	    }
	    ...
	    if(null!==$this->input->server('REQUEST_METHOD') && $this->input->server('REQUEST_METHOD') == 'POST'){
			...
			$get = $this->model_knowledge->edit_knowledge($data,$id);
			if($get){
	    ...
	    $this->data['info'] = $this->db->where('id',$this->uri->segment(3))->get('knowledge')->row_array();
		$this->data['title'] = 'knowledge Edit';
	...
	// start model knowledge
	public function knowledge_list(){
		$val = $this->input->get('daterange-input');
		$get = $this->model_knowledge->knowledge_list($val);
	...
	public function knowledge_del(){
		$id = $this->input->post('id');
		$get = $this->model_knowledge->knowledge_del($id);
	...
	public function knowledge_delmulti(){
		if(empty($this->input->post('chk_knowledge'))){
			return false;
		}

		$id = $this->input->post('chk_knowledge');
		$del = $this->model_knowledge->knowledge_delmulti($id);
		if($del){
			echo 'success';
		}else{
			echo 'error';
	...
	public function knowledge_order_by(){
		$now = date("Y-m-d H:i:s");// set date to update
		$id   = $this->input->post('id');
		...
		$get = $this->model_knowledge->order_by($data,$id);
		if($get){
			echo 'success';
		}else{
			echo 'error';
	...
	public function knowledge_status(){
		$now = date("Y-m-d H:i:s");// set date to update
		$id   = $this->input->post('id');
		...
		$get = $this->model_knowledge->knowledge_status($data,$id);
		if($get){
			echo 'success';
		}else{
			echo 'error';
	...
	public function upload(){
        ...
        else
        {
          ...
          // 		'hash' => $this->hash
		    ...
		    $response = $this->model_knowledge->upload_fl($data);
		    if($response){
	...
	public function select(){
		$response = $this->model_knowledge->select_fl();
		if($response){
	...
	public function delete(){
		...
		if(unlink($path_del)){
			$this->model_knowledge->delete_fl($id);
		}
	}
