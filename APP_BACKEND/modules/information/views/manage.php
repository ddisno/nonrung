<!-- Content Wrapper. Contains page content -->
  <div class="content-wrapper">
    ...
    <section class="content-header">
      <h1>
       <?=$header?>
       <a class="btn btn-primary" href="<?=base_url();?>information/create"><i class="fa fa-plus"></i> เพิ่มข้อมูลพื้นฐาน</a>
      </h1>
      <ol class="breadcrumb">
  ...
  <script type="text/javascript">
    $(function(){
      ...
      var table = $('#table').DataTable( {
        ...
        "columns": [          
            { "data": function (data, type, row, meta) {
              return '<input type="checkbox" class="btn-dlm" name="chk_information[]" value="'+data.id+'"> ';
            }},
            { "data": "title" },
            { "data": function(data){
            ...
            { "data":function(data){
              if(data.status!=0){
                return "<input type='checkbox' class='show_information' checked>";
              }else{
                return "<input type='checkbox' class='show_information' >";
              }      
            } },
            {"data": function(data, type, row, meta){
              return   "<div class='btn-group'>"+
              "<a href='<?=base_url();?>information/edit/"+data.id+" ' class='btn btn-default btn-sm edit'><i class='fa fa-edit'></i></a>"+
      ...
      function cb(start='', end='') { 
        ...
        if(start==''){
          table.ajax.url('<?=base_url();?>information/information_list').load();   
          ...
          table.ajax.url('<?=base_url();?>information/information_list/?'+$('#daterange').serialize()).load(); 
        }     
      }
        ...
        swal({
          ...
          preConfirm: function () {
            return new Promise(function (resolve) {
              $.ajax({
                url: '<?=base_url();?>information/information_del',
                type: 'POST',
      ...
      $(document).on('click','#delete_multi',function(){
        ...
        if(!$('.btn-dlm').is(':checked')){
          swal('Warning','Please choose information to delete','warning');
          return false;
        }
        ...
        swal({
          ...
          preConfirm: function () {
            return new Promise(function (resolve) {
              $.ajax({
                url: '<?=base_url();?>information/information_delmulti',
                type: 'POST',
                data: form_data,
                success:function(data){
                  console.log(data);
      ...
      $(document).on('change', '.order_by', function(event) {
        ...
        var order_by = $(this).val();
              ...
              $.ajax({
                url: '<?=base_url();?>information/information_order_by',
                type: 'POST',
      ...
      $(document).on('click', '.show_information', function(event) {
        var status = 0;
        var data = table.row( $(this).parents('tr') ).data();
        if(!$(this).is(':checked')){
          status = 0;
        ...
        $.ajax({
          url: '<?=base_url();?>information/information_status',
          type: 'POST',
      ...
      $(document).on('click', '.show_stick', function(event) {
        ...
        $.ajax({
          url: '<?=base_url();?>information/information_stick',
          type: 'POST',
          data: {data:status,id:data.id,csrf_token_name:$('input[name="csrf_token_name"]').val()},
          success:function(data){
            console.log(data);
