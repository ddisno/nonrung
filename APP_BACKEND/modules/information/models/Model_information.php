<?php
	...
	class Model_information extends CI_Model
	{
		...
		public function __construct(){
			// check table
			$this->load->dbforge();
			$this->information_table();
		}

		public function information_list($val){
			$this->db->select('*');
			...
			if($val!=''){
				...
				$this->db->where('information.create_datetime >=',$date_start);
				$this->db->where('information.create_datetime <=',$date_end);
			}
		...
		public function save_information($data){
			$get = $this->db->insert('information',$data);
			if($get){
				return true;
			}else{
				return false;
		...
		// ลบบล็อก--------------
		public function information_del($id){
			$this->db->where('id',$id);
		...
		public function information_status($data,$id){
			$this->db->where('id',$id);
			$query = $this->db->update('information',$data);
			if($query){
				return true;
		...
		public function information_delmulti($data){
			if (!empty($data)) {
		...
		public function information_status_home($data,$id){
			$this->db->where('id_cat',$id);
			$query = $this->db->update('information_cat',$data);
			if($query){
				return true;
			}else{
				return false;
		...
		public function information_table(){
			...
			$this->dbforge->add_field($fields_information);
			...
			$this->dbforge->create_table('information',TRUE,$attributes);

			// $this->db->query('ALTER TABLE information ADD FOREIGN KEY(id_cat) REFERENCES information_cat(id_cat) ON DELETE RESTRICT ON UPDATE RESTRICT;');
		}
	// end exsit table
	}
?>