...
class Law extends MY_Controller
{
	...
	public function index(){
		...
		$this->data['title'] = 'กฏหมายน่ารู้';
		$this->data['header'] = 'กฏหมายน่ารู้';
		$this->middle = 'manage';
		$this->layout();
	}
	...
	public function create($id=''){
		if(null!==$this->input->server('REQUEST_METHOD') && $this->input->server('REQUEST_METHOD') == 'POST'){
			...
			$get_order = $this->model_law->get_order_law();
			if($get_order){
	        ...
	        $get = $this->model_law->save_law($data);
	        if($get){
		...
		$this->data['title'] = 'law Create';
		$this->data['header'] = 'เพิ่มกฏหมายน่ารู้';
		$this->middle = 'create';
		$this->layout();
	}

	public function edit($id=''){
		if($get = $this->db->where('id',$this->uri->segment(3))->get('law')->num_rows()==''){
	      redirect('law');
	    }
	    ...
	    if(null!==$this->input->server('REQUEST_METHOD') && $this->input->server('REQUEST_METHOD') == 'POST'){
			...
			$get = $this->model_law->edit_law($data,$id);
			if($get){
	    ...
	    $this->data['info'] = $this->db->where('id',$this->uri->segment(3))->get('law')->row_array();
		$this->data['title'] = 'law Edit';
	...
	public function law_list(){
		...
		$get = $this->model_law->law_list($val);
	...
	public function law_delmulti(){
		if(empty($this->input->post('chk_law'))){
			return false;
		}

		$id = $this->input->post('chk_law');
		$del = $this->model_law->law_delmulti($id);
		if($del){
			echo 'success';
		}else{
			echo 'error';
	...
	public function law_order_by(){
		...
		$get = $this->model_law->order_by($data,$id);
		if($get){
			echo 'success';
		}else{
			echo 'error';
	...
	public function law_status(){
		...
		$get = $this->model_law->law_status($data,$id);
		if($get){
			echo 'success';
		}else{
			echo 'error';
	...
	public function upload(){
        ...
        else
        {
          ...
          // 		'hash' => $this->hash
		    ...
		    $response = $this->model_law->upload_fl($data);
		    if($response){
	...
	public function select(){
		$response = $this->model_law->select_fl();
		if($response){
