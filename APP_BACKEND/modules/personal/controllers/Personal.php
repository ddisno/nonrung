...
class Personal extends MY_Controller
{
	...
	public function index(){
		...
		$this->data['header'] = 'ทำเนียบบุคลากร';
		$this->middle = 'manage';
		$this->layout();
	}
	...
	public function create($id=''){
		if(null!==$this->input->server('REQUEST_METHOD') && $this->input->server('REQUEST_METHOD') == 'POST'){
			...
			$get_order = $this->model_personal->get_order_personal();
			if($get_order){
	        ...
	        $get = $this->model_personal->save_personal($data);
	        if($get){
		...
		$this->data['title'] = 'Personal Create';
		$this->data['header'] = 'เพิ่มทำเนียบบุคลากร';
		$this->middle = 'create';
		$this->layout();
	}

	public function edit($id=''){
		if($get = $this->db->where('id',$this->uri->segment(3))->get('personal')->num_rows()==''){
	      redirect('personal');
	    }
	    ...
	    if(null!==$this->input->server('REQUEST_METHOD') && $this->input->server('REQUEST_METHOD') == 'POST'){
			...
			$get = $this->model_personal->edit_personal($data,$id);
			if($get){
	    ...
	    $this->data['info'] = $this->db->where('id',$this->uri->segment(3))->get('personal')->row_array();
		$this->data['title'] = 'personal Edit';
	...
	public function personal_del(){
		$id = $this->input->post('id');
		$get = $this->model_personal->personal_del($id);
	...
	public function personal_delmulti(){
		if(empty($this->input->post('chk_personal'))){
			return false;
		}

		$id = $this->input->post('chk_personal');
		$del = $this->model_personal->personal_delmulti($id);
		if($del){
			echo 'success';
		}else{
			echo 'error';
	...
	public function personal_order_by(){
		$now = date("Y-m-d H:i:s");// set date to update
		$id   = $this->input->post('id');
		...
		$get = $this->model_personal->order_by($data,$id);
		if($get){
			echo 'success';
		}else{
			echo 'error';
	...
	public function personal_status(){
		...
		$get = $this->model_personal->personal_status($data,$id);
		if($get){
			echo 'success';
		}else{
			echo 'error';
	...
	public function upload(){
        ...
        else
        {
          ...
          // 		'hash' => $this->hash
		    ...
		    $response = $this->model_personal->upload_fl($data);
		    if($response){
	...
	public function select(){
		$response = $this->model_personal->select_fl();
		if($response){
	...
	public function delete(){
		...
		if(unlink($path_del)){
			$this->model_personal->delete_fl($id);
		}
	}
