<?php
	...
	class Model_purchase extends CI_Model
	{
		...
		public function purchase_list($val){
			$this->db->select('*');
			...
			if($val!=''){
				...
				$this->db->where('purchase.create_datetime >=',$date_start);
				$this->db->where('purchase.create_datetime <=',$date_end);
			}
		...
		public function save_purchase($data){
			$get = $this->db->insert('purchase',$data);
			if($get){
				return true;
			}else{
				return false;
		...
		// ลบบล็อก--------------
		public function purchase_del($id){
			$this->db->where('id',$id);
		...
		public function purchase_status($data,$id){
			$this->db->where('id',$id);
			$query = $this->db->update('purchase',$data);
			if($query){
				return true;
		...
		public function purchase_delmulti($data){
			if (!empty($data)) {
		...
		public function purchase_status_home($data,$id){
			$this->db->where('id_cat',$id);
			$query = $this->db->update('purchase_cat',$data);
			if($query){
				return true;
			}else{
				return false;
		...
		public function purchase_table(){
			...
			$this->dbforge->add_field($fields_purchase);
			...
			$this->dbforge->create_table('purchase',TRUE,$attributes);

			// $this->db->query('ALTER TABLE purchase ADD FOREIGN KEY(id_cat) REFERENCES purchase_cat(id_cat) ON DELETE RESTRICT ON UPDATE RESTRICT;');
		}
	// end exsit table
	}
?>