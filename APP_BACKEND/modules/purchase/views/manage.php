<!-- Content Wrapper. Contains page content -->
  <div class="content-wrapper">
    ...
    <section class="content-header">
      <h1>
       <?=$header?>
       <a class="btn btn-primary" href="<?=base_url();?>purchase/create"><i class="fa fa-plus"></i> เพิ่มจัดซื้อจัดจ้าง</a>
      </h1>
      <ol class="breadcrumb">
  ...
  <script type="text/javascript">
    $(function(){
      ...
      var table = $('#table').DataTable( {
        ...
        "columns": [          
            { "data": function (data, type, row, meta) {
              return '<input type="checkbox" class="btn-dlm" name="chk_purchase[]" value="'+data.id+'"> ';
            }},
            { "data": "title" },
            { "data": function(data){
            ...
            { "data":function(data){
              if(data.status!=0){
                return "<input type='checkbox' class='show_purchase' checked>";
              }else{
                return "<input type='checkbox' class='show_purchase' >";
              }      
            } },
            {"data": function(data, type, row, meta){
              return   "<div class='btn-group'>"+
              "<a href='<?=base_url();?>purchase/edit/"+data.id+" ' class='btn btn-default btn-sm edit'><i class='fa fa-edit'></i></a>"+
      ...
      function cb(start='', end='') { 
        ...
        if(start==''){
          table.ajax.url('<?=base_url();?>purchase/purchase_list').load();   
          ...
          table.ajax.url('<?=base_url();?>purchase/purchase_list/?'+$('#daterange').serialize()).load(); 
        }     
      }
        ...
        swal({
          ...
          preConfirm: function () {
            return new Promise(function (resolve) {
              $.ajax({
                url: '<?=base_url();?>purchase/purchase_del',
                type: 'POST',
      ...
      $(document).on('click','#delete_multi',function(){
        ...
        if(!$('.btn-dlm').is(':checked')){
          swal('Warning','Please choose purchase to delete','warning');
          return false;
        }
        ...
        swal({
          ...
          preConfirm: function () {
            return new Promise(function (resolve) {
              $.ajax({
                url: '<?=base_url();?>purchase/purchase_delmulti',
                type: 'POST',
                data: form_data,
                success:function(data){
                  console.log(data);
      ...
      $(document).on('change', '.order_by', function(event) {
        ...
        var order_by = $(this).val();
              ...
              $.ajax({
                url: '<?=base_url();?>purchase/purchase_order_by',
                type: 'POST',
      ...
      $(document).on('click', '.show_purchase', function(event) {
        var status = 0;
        var data = table.row( $(this).parents('tr') ).data();
        if(!$(this).is(':checked')){
          status = 0;
        ...
        $.ajax({
          url: '<?=base_url();?>purchase/purchase_status',
          type: 'POST',
      ...
      $(document).on('click', '.show_stick', function(event) {
        ...
        $.ajax({
          url: '<?=base_url();?>purchase/purchase_stick',
          type: 'POST',
          data: {data:status,id:data.id,csrf_token_name:$('input[name="csrf_token_name"]').val()},
          success:function(data){
            console.log(data);
